...
const checkPassword = async (req, res) => {
  const { password, curUserPassword } = req.body;
  const isPasswordMatch = await bcrypt.compare(password, curUserPassword);
  if (isPasswordMatch) {
    res.status(201).json({ rightPassword: 1 });
  } else {
    res.status(400).json({ rightPassword: 0 });
  }
};
